...
import { EntityArrayResponseType, ApprovisionnementService } from '../service/approvisionnement.service';
...
export class ApprovisionnementComponent implements OnInit {
  ...
  trackId = (_index: number, item: IApprovisionnement): number => this.approvisionnementService.getApprovisionnementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.approvisionnements = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IApprovisionnement[] | null): IApprovisionnement[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const approvisionnementsNew = this.approvisionnements ?? [];
      if (data) {
        for (const d of data) {
          if (approvisionnementsNew.map(op => op.id).indexOf(d.id) === -1) {
            approvisionnementsNew.push(d);
          }
        }
      }
      return approvisionnementsNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.approvisionnementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
